...
解説はソースにも diff 上にも書かず、PR の会話欄に `<!-- pr-docs -->` で始まるコメントを 1 つだけ置く(文章 20 行以内。構造が変わったものだけ ERD / シーケンス / クラス / フローの Before/After を Mermaid で描く。読む順番、注意点)。2 回目以降は同じコメントを書き換える。本文の書き直しは、ユーザー環境に `c-refresh-pr` スキルがあればそれを使う(squash はしない)。
