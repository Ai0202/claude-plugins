...
対象は「比較元ブランチとの分岐点以降のコミット + 未コミット変更 + 未追跡の新規ファイル」(計測ログと `.claude/*.local.md` は除く)。比較元は次の順で自動判定する:
...
- **変更障害率が下がる** → 上の指標が実際の障害減少につながっている(最終的な成果指標)。数字に意味を持たせる運用は 2 つ: **PR に種別ラベル(`type:feature` / `type:bugfix` / `type:hotfix` / `type:chore`)を 1 つ付ける**、**不具合を直す PR の本文に `Caused-by: #<原因PR>`(調べて該当なしなら `Caused-by: unknown`)を 1 行書く**。/atdd の PR ステップはどちらも自動で行う。原因が期間外の古い不具合は数えないので、直近のリリース品質だけが出る
