...
echo "PR #${PR_NUM} の説明が最新のコミット(${HEAD_SHA:0:7})に追いついていません。タスクを終える前に /pr-docs を実行して、(1) 実装内容から PR のタイトル・本文をテンプレートに沿って書き直し、(2) Before/After の図つき「変更の読み方」を PR のコメントとして付けて(既にあれば更新して)ください。ユーザーへの質問で止まる場合は、その質問だけを再提示して止まって構いません。" >&2
exit 2
