---
description: 実装内容から PR のタイトル・本文をテンプレートに沿って書き直し、Before/After の図(ERD・シーケンス・クラス・フロー)つきの「変更の読み方」を PR のコメントとして 1 つ付ける(2回目以降は同じコメントを更新)
allowed-tools: Bash, Read, Grep, Glob, Skill
---

現在のブランチの PR を、実際にコミットされた内容に合わせて仕上げてください。引数: $ARGUMENTS(PR 番号。省略時は現在のブランチの PR)

やることは 2 つ。**(1) PR のタイトル・本文の書き直し**、**(2) Before/After の図つき「変更の読み方」を PR コメントで 1 つ**。コードは一切変更しない。
...
### 3. 「変更の読み方」を PR のコメントとして 1 つ付ける
想定する読み手は「このリポジトリに入ったばかりで、この変更の背景を知らないメンバー」。ただし **PR 上では読み手の属性(新卒向け・初心者向けなど)を一切書かない**。見出し・本文・まとめ文のどこにも入れない。diff の行に付けるインラインコメントではなく、**PR の会話欄(Conversation)に 1 つのコメント**として書く。ソースコードにも書かない。

**内容(図が主、文は従)**。先頭に `<!-- pr-docs -->` と `## 📘 変更の読み方` を置く。文章部分は合計 20 行以内、図は必要な種類だけ:
