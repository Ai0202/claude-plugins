#!/usr/bin/env bash
# mark-design-passed.sh — 現在の差分が Figma との突合(/design-check)に合格したことを記録する
# (review-loop の pr-ready-gate.sh と同一ロジックで差分ハッシュを計算し、.git/ 内にマーカーを書く)
# 使い方: mark-design-passed.sh [ベースブランチ]
set -euo pipefail
...
MARKER="$(git rev-parse --git-dir)/claude-design-passed"
echo "$HASH" > "$MARKER"
echo "デザイン突合の合格を記録しました(${BASE_LINE#\# }): $MARKER"
