...
| **M** | 通常の機能追加・改修。単一リポジトリ | 入力に曖昧さがあるときだけ grill-me。設計は「影響範囲と構成」を 5〜10 行。ADR は覆すと高くつく決定があるときだけ | 全項目(ADR は該当時) |
| **L** | 複数リポジトリ、スキーマ / API の契約変更、認証・決済、新しい外部依存、新機能の初回 | grill-me 必須。設計・ADR を書いてから計画。区切りごとに `<atdd>PAUSE</atdd>` で確認を挟む | 全項目 |
...
1. **仕様の詰め**(L は必須、M は曖昧なときだけ、S はしない): grill 系スキル(`mattpocock-skills:grilling`。無ければ他の grill-me 相当)が使えるなら Skill ツールで呼び、曖昧な点・境界・やらないことをユーザーに問い詰めて固める。無ければ test-plan の質問ルール(成否に関わる不明点だけ、最大 3 問)で代用
