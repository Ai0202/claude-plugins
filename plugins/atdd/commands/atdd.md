...
|---|---|---|---|
| **S** | バグ修正・文言・設定・1 関数の変更。差分 50 行未満の見込み。スキーマ / API / 画面構成は変えない。単一リポジトリ | grill-me なし。設計・ADR なし。テスト計画は **再現テスト 1〜3 本**(バグなら「直す前に落ちるテスト」)。承認は計画を見せて一言でよい | 概要・テスト計画・チェックリスト・関連 PR・ログ |
...
1. **仕様の詰め**(L は必須、M は曖昧なときだけ、S はしない): `grill-me` 系スキル(例: `mattpocock-skills:grill-me`)が使えるなら Skill ツールで呼び、曖昧な点・境界・やらないことをユーザーに問い詰めて固める。無ければ test-plan の質問ルール(成否に関わる不明点だけ、最大 3 問)で代用
