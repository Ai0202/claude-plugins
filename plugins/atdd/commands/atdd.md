...
## 設計            grilling で固めた仕様、変更の地図(ディレクトリツリー: 触る/作るファイルと役割)、境界と依存(Mermaid 可)、影響範囲、やらないこと
...
2. **設計**(M / L): 固まった仕様を「設計」節の形に整理する。S は書かない。書く内容:
   - **変更の地図(ファイル構成)**: 触るファイル・新しく作るファイルをディレクトリツリーで示し、各行に「役割 / 新規 or 変更」を 1 行添える。置き場所はリポジトリの既存規約(CLAUDE.md、`.claude/rules/`、近い既存機能のディレクトリ)に合わせ、**似た機能が既にどこにあるかを Grep で確かめてから**決める。新しいディレクトリを切るときは理由を書く
   - **境界と依存**: 新しいモジュールの公開インターフェース(関数・型・API の形)、どの層がどの層を呼ぶか。構成が変わるなら Mermaid で描く
   - **影響範囲**: 変更する既存関数の呼び出し元、DB・API・画面への影響
   - **やらないこと**
   - `mattpocock-skills:codebase-design`(モジュールの境界・深さの設計)や `domain-modeling`(用語・ドメインモデル)が使えるなら、L と、M で新しいモジュールを切るときに Skill ツールで呼んで設計の質を上げる
   - 設計は承認の対象。ファイル構成に異論があればここで直す(RED 以降は構成を動かさない)
